...
py
```
Below are the $\mu$, $\sigma^2$, and $\sigma$, respectively:
```{r}
mu<-sum(y*py)
mu
EY2<-sum(y^2*py)
...
ggplot(aes(x=y, y=probs), data=dd) + geom_bar(stat='identity') + ylab('P(Y=y)') + 
ggtitle('pmf of Y2 from Example 2')
```
...
one.Y <- function() {
  ...
  red.roll <- sample(sample.space,1)
  white.roll <- sample(sample.space,1)
  num.rolled <- c(red.roll,white.roll)
  max.final <- max(num.rolled)
  return(max.final)
  }
one.Y()
```
...
12. (1pt) Note that your simulated results should be similar to the theoretical quantities; if they aren't, re-check your R code!  What is the reason for any differences?
  ...
  Besides that I am not quite sure how "set.seed" influences our results, the differences exist because of relatively small sample size (we only simulated 1000 pair of rolls). If we let the sample size, n, goes to infinity, our observed data will be closer enough to what we initially expected.
